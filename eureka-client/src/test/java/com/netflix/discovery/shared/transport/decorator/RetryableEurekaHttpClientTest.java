...
public class RetryableEurekaHttpClientTest {
    ...
    public void testRequestsReuseSameConnectionIfThereIsNoError() throws Exception {
        when(clientFactory.newClient(ArgumentMatchers.any())).thenReturn(clusterDelegates.get(0));
        ...
        verify(clientFactory, times(1)).newClient(ArgumentMatchers.any());
    ...
    public void testRequestIsRetriedOnConnectionError() throws Exception {
        when(clientFactory.newClient(ArgumentMatchers.any())).thenReturn(clusterDelegates.get(0), clusterDelegates.get(1));
        ...
        verify(clientFactory, times(2)).newClient(ArgumentMatchers.any());
    ...
    private void simulateTransportError(int delegateFrom, int count) {
        for (int i = 0; i < count; i++) {
            ...
            when(clientFactory.newClient(ArgumentMatchers.any())).thenReturn(clusterDelegates.get(delegateId));
